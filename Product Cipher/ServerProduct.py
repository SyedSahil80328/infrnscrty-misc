...
from RailFence import decodeFence
...
def handleClient(clientSocket):
    ...
    keyMatrix = json.loads(data["keyMatrixData"])

    print(f"Received Cipher Text: {cipherText}")

    det = determinant(keyMatrix)
    tempPlainText = decode(cipherText, keyMatrix, det)
    plainText = decodeFence(tempPlainText, data["keyValue"])
