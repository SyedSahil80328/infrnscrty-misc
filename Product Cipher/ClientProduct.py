...
from RailFence import encodeFence
...
def startClient():
    ...
    keyValue = int(input("Enter the Key value (for Rail Fence Cipher): "))
    tempCipherText = encodeFence(plainText, keyValue)

    keyLengths = findFactors(len(tempCipherText))
    ...
    key = input("Enter a key (for Hill Cipher): ").upper()
    keyLength = len(key)
    ...
    while keyLength not in keyLengths or (det%2 == 0 or det%13 == 0):
        ...
        key = input("Enter proper key: ").upper()
        keyLength = len(key)
        keyMatrix = convertKeyToMatrix(key)
        det = abs(determinant(keyMatrix))

    cipherText = encode(tempCipherText, keyMatrix)
    keyMatrixData = json.dumps(keyMatrix)
    data = {"cipherText": cipherText, "keyMatrixData": keyMatrixData, "keyValue": keyValue}
